...
- **Switch (Toggle)**: track ON `background var(--accent)`, OFF `background var(--surface)`, both with `box-shadow inset 0 0 0 1px var(--ink4)`, knob inset `2px`. The outline is an **inset shadow, never a border**: a border is part of the box, so under `border-box` it eats a pixel off the padding box that the knob's `left`/percentage `top` resolve against — it shrinks the knob's surround from 2px to 1px on all four sides and lands the ON and OFF edges on different device-pixel boundaries at a fractional DPR, which reads as the dot changing size when you flip it. A shadow paints over the track's edge and takes part in no layout. **One outline token for both states, and it is `--ink4`, because a switch owes 3:1 at its boundary (WCAG 1.4.11) and nothing else in the ramp delivers it.** The measurements, swept across every System × Mode × Accent by `contrast.test.ts`: the OFF fill against the row behind it is 1.03–1.16:1 (a fill is not an edge); `--border2`, the ramp's nominal "strong edge", is 1.42–1.84:1 at the default Contrast and still only 2.82–4.29:1 at **High**, so a switch outlined in it is invisible at every level the app offers — it shipped, and the report back was that nothing had changed; `--accent` fails from the other side, since a pale accent on a light theme puts the ON fill at 1.36:1, making an accent-coloured outline a line drawn in the colour beneath it. `--ink4` is the lowest neutral `boost()` floors at 4.5:1 at every Contrast level and measures 4.77–6.76:1 against both `--surface` and `--panel` everywhere. Being neutral it is mode-relative for free (dark line on a light page, light on a dark one) and independent of the accent — which is why it is written outside the `checked` branch rather than duplicated into it. Knob ON `background var(--accent-ink)`, OFF `background var(--ink4)`. The knob's two states use **different** tokens on purpose: `--accent-ink` is calibrated only against the accent fill it sits on when ON, and has no contrast contract anywhere else — painted unconditionally it drew the OFF knob at 1.0–1.15:1, and under the `mono` accent `--accent-ink` and `--bg` are the same literal. Any control whose state changes the surface beneath a part must re-pick that part's token with it. Disabled: `opacity` on the wrapper — the switch has no disabled colour branch, so unlike Button this alpha is its only inert cue and cannot simply be dropped.
