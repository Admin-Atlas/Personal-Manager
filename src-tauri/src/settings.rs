...
// The duplicate check used to be gated by a `duplicate_check_enabled` setting, read here. It is
// gone (#711): the check lives in the Documents toolbar beside Rebuild, where it is one faint button
// next to the thing it acts on, and a preference governing whether a button appears was a control
// with no home of its own — "one control, one home". Any stored row is simply never read again;
// deleting it would be a migration to no purpose.
