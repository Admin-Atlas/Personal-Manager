...
export const HELP: Record<string, HelpEntry> = {
  ...
  "pinboard-folder-game": {
    ...
    body: "Can't decide what to do next? Put the candidates in a folder, press the dice, and pick a game: a roulette wheel, a fist of straws, or a box of folded slips. Each has a piece for every note in the folder, and each picks one completely at random — the difference is only the theatre. Whatever it lands on is your next task. A card it has already picked stays in the folder but greys out and isn't offered again until every other card has had its turn, at which point the round loops all the way back and starts fresh. That round is remembered, so you can close PM, come back later, and carry on where you left off rather than being handed the same job twice. Turn on 'Move the winner out' and a drawn card leaves for the main board instead of staying here. Timelines can sit in a game folder but are never drawn — a dated track isn't a task. If motion is turned down in Settings, the games skip straight to the answer.",
  },
