...
export const HELP: Record<string, HelpEntry> = {
  ...
  "dev-sandbox": {
    ...
    body: "The document engine parses your files in a background helper. On Windows that helper runs inside a locked-down OS sandbox with no network access, able to read only a handful of folders (the engine's own files and one file at a time as it's processed) — so a malicious file can't reach the internet or your vault. This panel shows whether that confinement is active; if it couldn't be set up the engine still runs (just unconfined) and the reason is shown here with a diagnostic code like SBX-2104. If you report a problem, include that code — it points us at the exact spot; the full list is in ERROR_CODES.md in the GitHub repo. In a development build, the network self-test button asks the helper to try one outbound connection and confirms the block is real.",
  },
  "dev-calendar": {
